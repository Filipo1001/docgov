 ...
 *
 * EL ICONO SIEMPRE ESTÁ, EL PORCENTAJE ACOMPAÑA. La versión anterior sustituía
 * el icono por el número cuando conocía el progreso, así que las subidas con
 * porcentaje perdían la única pista de qué se estaba subiendo. Aquí el icono se
 * queda siempre en el centro y el porcentaje va en el texto. Importa porque
 * solo tres de los doce puntos conocen los bytes: si el diseño dependiera del
 * número, nueve pantallas se verían de segunda.
 */
...
export default function SubiendoArchivo({
  ...
  etiqueta: string
  /** 0–100 cuando se conocen los bytes; omitido o null si no. */
  ...
  const determinado = typeof progreso === 'number' && Number.isFinite(progreso)
  const pct = determinado ? Math.min(100, Math.max(0, Math.round(progreso))) : 0
  ...
  return (
    <div
      ...
      role="status"
      aria-live="polite"
      aria-label={determinado ? `${etiqueta}, ${pct}%` : etiqueta}
    >
      <div className="bg-white rounded-3xl px-10 py-8 flex flex-col items-center gap-5 shadow-2xl mx-6 w-full max-w-xs upload-card-enter">
        ...
        <div className="relative w-24 h-24">
          ...
          {/* Arco de progreso. Con bytes conocidos avanza; sin ellos gira. */}
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-gray-900">
            {etiqueta}
            {determinado && <span className="tabular-nums text-gray-500"> · {pct}%</span>}
          </p>
