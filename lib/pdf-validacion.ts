...
/** Límites del anexo. El de archivo coincide con el del bucket (defensa en dos capas). */
 ...
 */
export async function validarPDF(buf: Buffer): Promise<ResultadoValidacion> {
  ...
  if (buf.length > ADJUNTO_MAX_BYTES) {
    return { ok: false, error: `El archivo supera el máximo de ${Math.round(ADJUNTO_MAX_BYTES / 1024 / 1024)} MB.` }
  }
  ...
  try {
    ...
    if (paginas > ADJUNTO_MAX_PAGINAS) {
      return { ok: false, error: `El PDF tiene ${paginas} páginas; el máximo permitido es ${ADJUNTO_MAX_PAGINAS}.` }
    }
