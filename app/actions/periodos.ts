    ...
    after(async () => {
      try {
        ...
        const notifBase = {
          ...
          // Campana sí, correo no. Un envío avisa al supervisor y a todos los
          // asesores de la dependencia: con 97 informes al mes son ~380
          // correos, y el 82% caen en ocho días. La supervisora de la
          // secretaría grande recibía sesenta en una semana — nadie lee eso,
          // se archiva en bloque, y de paso entierra los avisos que sí piden
          // una acción. Quien revisa recibe ahora UN resumen diario (R2 del
          // cron), que además dice cuáles llevan esperando demasiado.
          sinCorreo: true,
        }
...
export async function enviarRecordatorioInforme(periodoId: string): Promise<ActionResult> {
  try {
    ...
    await enviarNotificacion({
      ...
      mensaje: `Aún no has enviado tu informe de ${periodo.mes} ${periodo.anio} para el contrato ${contrato.numero}. Ingresa a Contratista Digital para enviarlo.`,
      periodoId,
      mes: periodo.mes,
      anio: periodo.anio,
      contrato: contrato.numero || '',
    ...
    await Promise.allSettled(
      periodos.map(async (p) => {
        ...
        await enviarNotificacion({
          ...
          mensaje: `Aún no has enviado tu informe de ${p.mes} ${p.anio} para el contrato ${contrato.numero}. Ingresa a Contratista Digital para enviarlo.`,
          periodoId: p.id,
          mes: p.mes,
          anio: p.anio,
          contrato: contrato.numero || '',
