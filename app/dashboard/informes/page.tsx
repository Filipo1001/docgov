...
function InformeCard({
  ...
  async function handleRevocarAsesor() {
    setProcesando(true)
    const res = await rechazarComoAsesor(periodo.id, 'Aprobación revocada por asesor')
    if (res.error) toast.error(res.error)
    else { toast.success('Aprobación revocada'); onUpdate() }
    setProcesando(false)
  }
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="text-right flex-shrink-0">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${ESTADO_COLOR[periodo.estado]}`}>
    ...
    <div className="max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            {esAsesor ? 'Informes de los contratistas de tu dependencia' :
             esSecretaria ? 'Informes de todos los contratistas' :
             'Informes mensuales'}
          </p>
        </div>
      ...
      {!cargando && periodos.length > 0 && (
        <div className="grid grid-cols-4 gap-3 mb-6">
          ...
          <div className="bg-white rounded-xl border px-4 py-3 text-center">
            <p className="text-2xl font-bold text-blue-600">{enviados.length}</p>
            <p className="text-xs text-gray-400">Enviados</p>
          </div>
          <div className="bg-white rounded-xl border px-4 py-3 text-center">
            <p className="text-2xl font-bold text-indigo-600">{aprobadosAsesor.length}</p>
            <p className="text-xs text-gray-400">Aprobados asesor</p>
          </div>
          <div className="bg-white rounded-xl border px-4 py-3 text-center">
            <p className="text-2xl font-bold text-emerald-600">{aprobados.length}</p>
            <p className="text-xs text-gray-400">Aprobados final</p>
          </div>
        </div>
      )}
          ...
          {([
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg transition-colors ${
                filtro === f.key ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'
              }`}
            >
              {f.label} ({f.count})
            </button>
          ...
          <div className="ml-auto relative">
            ...
            {menuAbierto && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl border shadow-lg z-20 py-1">
      ...
      {cargando ? (
        ...
        <div className="bg-white rounded-2xl border p-12 text-center">
          <div className="text-4xl mb-3">{filtro === 'todos' ? '📭' : '🔍'}</div>
          <h3 className="font-semibold text-gray-900 mb-1">
            {filtro === 'todos' ? 'Sin informes este mes' : 'Sin resultados'}
          </h3>
