...
export default function PeriodoDetallePage({
  ...
  // Panel de descargas del pipeline (Opción B — nodo final accionable)
  const [descargasPipelineAbierto, setDescargasPipelineAbierto] = useState(false)
  ...
  // ── Descargas del pipeline (Opción B) — filtradas por rol ──────────────────
  // Documentos disponibles según quién mira: el contratista descarga sus
  // entregables; asesor/supervisor descargan las actas institucionales.
  const documentosPipeline: { href: string; icono: string; nombre: string }[] = periodo
    ? esContratista
      ? [
          { href: `/api/pdf/${periodoId}/informe`,      icono: '📝', nombre: 'Informe de Actividades' },
          { href: `/api/pdf/${periodoId}/cuenta-cobro`, icono: '💰', nombre: 'Cuenta de Cobro' },
          ...(periodo.planilla_ss_url
            ? [{ href: periodo.planilla_ss_url, icono: '🏥', nombre: 'Planilla de Seguridad Social' }]
            : []),
        ]
      : (esAsesor || esSecretaria)
        ? [
            { href: `/api/pdf/${periodoId}/acta-supervision`, icono: '📋', nombre: 'Acta de Supervisión' },
            { href: `/api/pdf/${periodoId}/acta-pago`,        icono: '🧾', nombre: 'Acta de Pago' },
          ]
        : []
    : []
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-white rounded-2xl border p-5 mb-6">
        {rechazado ? (
          ...
          <div className="flex items-center gap-0">
            {STEPS.map((step, i) => {
              ...
              // Nodo accionable: Aprobado/Radicado ya alcanzado + hay documentos del rol.
              const esNodoDescargable =
                (step.estado === 'aprobado' || step.estado === 'radicado') &&
                i <= currentIdx &&
                puedeDescargarPaquete &&
                documentosPipeline.length > 0

              const circulo = (
                <div className={`relative w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  esNodoDescargable
                    ? 'bg-emerald-600 text-white ring-2 ring-emerald-300 ring-offset-1 group-hover:ring-emerald-400 group-hover:scale-110'
              ...
              const etiqueta = (
                <span className={`text-[9px] mt-1 font-medium text-center leading-tight ${
                  esNodoDescargable
                    ...
                    : done ? 'text-emerald-600 hidden sm:block'
                    : 'text-gray-400 hidden sm:block'
                }`}>
                  {esNodoDescargable ? 'Descargar' : step.short}
                </span>
              )
              ...
              return (
                <div key={step.estado} className="flex items-center flex-1 min-w-0">
                  {esNodoDescargable ? (
                    <button
                      type="button"
                      onClick={() => setDescargasPipelineAbierto(v => !v)}
                      aria-expanded={descargasPipelineAbierto}
        ...
        {/* Panel de descargas del pipeline — desplegable inline, filtrado por rol */}
        {puedeDescargarPaquete && documentosPipeline.length > 0 && descargasPipelineAbierto && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <p className="text-xs font-semibold text-gray-500 mb-2">
              {esContratista ? 'Tus documentos' : 'Documentos institucionales'}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {documentosPipeline.map((doc) => (
                <a
                  key={doc.href}
                  href={doc.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-3 px-3 py-2.5 bg-emerald-50 border border-emerald-100 rounded-xl hover:bg-emerald-100 transition-colors"
                >
                  <span className="text-lg shrink-0">{doc.icono}</span>
                  <span className="text-sm font-medium text-gray-900 flex-1 min-w-0 truncate">{doc.nombre}</span>
