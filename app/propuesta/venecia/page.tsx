...
/**
 * Propuesta comercial para la Alcaldía de Venecia.
 *
 * Vive en el ápice —contratistadigital.com/propuesta/venecia— y no en el
 ...
 *
 * ── Heredado de Dabeiba y El Bagre ───────────────────────────────────────
 *
 ...
 *
 * LAS CIFRAS VAN AL FINAL, justo antes de la llamada a la acción. La
 * atribución al municipio queda en una línea discreta: los números mandan,
 * pero sin origen valdrían mucho menos si alguien pregunta de dónde salen.
 *
 * ── Lo propio de esta propuesta ──────────────────────────────────────────
 *
 * DOS OPCIONES DE IMPLEMENTACIÓN, Y UNA RECOMENDADA. Dabeiba llevaba un solo
 * pago inicial; aquí el municipio elige quién carga los contratos. La opción
 * A la marca visualmente como recomendada, y no por margen: la B deja el
 * cargue en manos del equipo de contratación, que es justo el cuello de
 * botella de una alcaldía pequeña. Si el cargue se aplaza, la plataforma
 * queda vacía y a los dos meses parece que el software no sirve. Por eso el
 * diferenciador que se enuncia no es «nosotros digitamos» sino EL TIEMPO
 * HASTA EL PRIMER CICLO: es lo que de verdad cambia entre una y otra, y es
 * lo único que la B no puede prometer porque no depende de nosotros.
 *
 * LA ADAPTACIÓN DE DOCUMENTOS VA EN AMBAS. Las actas que genera la
 * plataforma llevan el nombre del municipio en su cuerpo legal, así que
 * adaptarlas no es opcional ni negociable: si faltara en la B quedaría un
 * compromiso implícito sin cobrar.
 *
 * CIFRAS AL DÍA. Las de Dabeiba se consultaron el 26 de agosto y ya
 * envejecieron: los documentos verificables pasaron de 269 a 503 en menos de
 * una semana. Aquí van las del 1 de septiembre de 2026. Una cifra vieja en
 * una propuesta vale menos que no ponerla, porque quien pregunte de dónde
 * sale se encuentra con otro número.
 *
 ...
 * es en móvil.
 *
 * EL ESCUDO ES OPCIONAL. Se dibuja solo si el archivo existe. El de Venecia
 * trae el nombre del municipio incorporado en la imagen —a diferencia del de
 * Dabeiba, que es solo el blasón—, así que convive con la línea «Propuesta
 * preparada para la Alcaldía Municipal de Venecia» sin competir: el escudo
 * va a 48 px y el texto en gris tenue, como sello y no como titular.
 */

export const metadata: Metadata = {
  title: 'Propuesta · Alcaldía de Venecia — Contratista Digital',
...
const ESCUDO = '/marca/escudo-venecia.png'
function hayEscudo(): boolean {
  try {
    return fs.existsSync(path.join(process.cwd(), 'public', 'marca', 'escudo-venecia.png'))
  } catch {
    return false
  }
}
...
export default async function PropuestaVenecia() {
  ...
  return (
    <main className="bg-white">
        ...
        <div className="max-w-3xl mx-auto w-full">
          ...
          <Revelar retraso={120}>
            <h1 className="mt-10 text-4xl sm:text-6xl font-bold tracking-tight leading-[1.08]">
              Que ningún contrato de Venecia vuelva a quedarse sin expediente completo.
            </h1>
          </Revelar>
          ...
          <Revelar retraso={400}>
            <div className="mt-12 sm:mt-16 flex items-center gap-3">
              {escudo && (
                ...
                <img
                  src={ESCUDO}
                  alt="Escudo del municipio de Venecia"
                  className="w-12 h-12 object-contain"
                />
              )}
              <p className="text-xs text-white/40 leading-snug">
                Propuesta preparada para la<br />Alcaldía Municipal de Venecia
              </p>
            </div>
          </Revelar>
        </div>
      ...
      <Seccion>
        ...
        {/* DOS CAMINOS DE IMPLEMENTACIÓN, UNA MENSUALIDAD.
            El municipio elige quién carga los contratos; todo lo demás es
            idéntico. La A se marca como recomendada porque la B deja el
            cargue en el equipo de contratación —el cuello de botella real de
            una alcaldía pequeña— y un cargue aplazado deja la plataforma
            vacía justo cuando hay que demostrar que sirve. */}
        <Revelar retraso={100} desde="zoom">
          <p className="mt-4 text-sm text-gray-500">
            Elija quién realiza el cargue inicial. La operación mensual es la misma
            en ambos casos.
          </p>
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <div className="relative rounded-2xl border-2 p-6 sm:p-7 text-center" style={{ borderColor: MARCA }}>
              <span
                className="absolute -top-2.5 left-1/2 -translate-x-1/2 rounded-full px-3 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-white"
                style={{ backgroundColor: MARCA }}
              >
                Recomendada
              </span>
              <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-400">
                Implementación A · pago único
              </p>
              <p className="mt-3 text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">
                $4.990.000
              </p>
              <p className="mt-2 text-sm font-medium" style={{ color: MARCA }}>
                Su primer ciclo mensual sale en la plataforma
              </p>
              <p className="mt-1 text-xs text-gray-500">
                Nosotros creamos los usuarios y cargamos los contratos
              </p>
            </div>
            <div className="rounded-2xl border border-gray-200 p-6 sm:p-7 text-center">
              <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-400">
                Implementación B · pago único
              </p>
              <p className="mt-3 text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">
                $3.990.000
              </p>
              <p className="mt-2 text-sm font-medium text-gray-700">
                El municipio realiza el cargue
              </p>
              <p className="mt-1 text-xs text-gray-500">
                Capacitamos a contratación y acompañamos el proceso
              </p>
            </div>
          </div>

          <div className="mt-4 rounded-2xl p-6 sm:p-7 text-center text-white" style={{ backgroundColor: MARCA }}>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white/50">
              Cada mes · en ambas opciones
            </p>
            <p className="mt-3 text-3xl sm:text-4xl font-bold tracking-tight">
              $2.400.000
            </p>
        ...
        <Revelar retraso={240}>
          <div className="mt-4 grid sm:grid-cols-2 gap-4">
            <div className="rounded-2xl border-2 p-6" style={{ borderColor: MARCA }}>
              <p className="text-sm font-semibold text-gray-900">La implementación A incluye</p>
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                {[
                  'Creación de los usuarios de todas las secretarías',
                  'Cargue de los contratos vigentes',
                  'Adaptación de los documentos al formato de Venecia',
                  ...
                  <div key={t} className="flex gap-2.5">
                    <span className="mt-1.5 w-1 h-1 rounded-full shrink-0" style={{ backgroundColor: MARCA }} />
                    <span>{t}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-gray-100 p-6">
              <p className="text-sm font-semibold text-gray-900">La implementación B incluye</p>
              <div className="mt-3 space-y-2 text-sm text-gray-600">
                {[
                  'Creación de los usuarios de todas las secretarías',
                  'Adaptación de los documentos al formato de Venecia',
                  'Capacitación al equipo de contratación para realizar el cargue',
                  'Capacitación por rol: contratistas y supervisores',
                  'Soporte durante el cargue que ejecuta el municipio',
                ].map(t => (
                  <div key={t} className="flex gap-2.5">
        ...
        <div className="max-w-3xl mx-auto">
          <Revelar>
            ...
            <p className="mt-8 text-2xl sm:text-3xl font-bold tracking-tight leading-snug">
              Hablemos de cómo se vería el primer ciclo en Venecia.
            </p>
          </Revelar>

          <Revelar retraso={200}>
            <a
              href={enlaceWhatsApp('Buen día. Soy de la Alcaldía de Venecia y quisiera conocer más sobre Contratista Digital.')}
              target="_blank"
          ...
          <Revelar retraso={320}>
            <p className="mt-16 text-xs text-white/30">
              Contratista Digital · Propuesta para la Alcaldía Municipal de Venecia
            </p>
          </Revelar>
        </div>
      </section>
